...
json PrintSymbolAddress(IDiaSymbol *pSymbol)
{
  ...
  if (pSymbol->get_relativeVirtualAddress(&dwRVA) == S_OK) {
    j["relativeVirtualAddress"] = dwRVA;
  } else {
    j["relativeVirtualAddress"] = nullptr;
  }

  pSymbol->get_addressSection(&dwSeg);
  j["addressSection"] = dwSeg;

  pSymbol->get_addressOffset(&dwOff);
  j["addressOffset"] = dwOff;
...
//
json PrintGlobalSymbol(IDiaSymbol *pSymbol)
{
  DWORD dwSymTag;
  DWORD dwRVA;
  DWORD dwSeg;
...
//
json PrintName(IDiaSymbol *pSymbol)
{
  json j;
  j["name"] = nullptr;
